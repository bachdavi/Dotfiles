...
state = status.split(": ")[2].split(", ")[0]
commasplitstatus = status.split(", ")
if state=="Unknown":
    percentleft = int(commasplitstatus[3].rstrip("%\n"))
if state=="Full":
    percentleft = int(commasplitstatus[3].rstrip("%\n"))
else:
    percentleft = int(commasplitstatus[2].rstrip("%\n"))

FA_CHR = "<span font='FontAwesome'>\uf077</span>"
FA_EQ = "<span font='FontAwesome'>\uf068</span>"
...
if state == "Discharging":
    ...
    form = ' <span color="{}">{}</span>'
    fulltext += form.format(color(percentleft), FA_EQ)
else:
    fulltext += " " + FA_FULL
